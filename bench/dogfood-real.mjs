...
const fields = await evaluate(`(() => {
  const out = [];
  const items = [...document.querySelectorAll('[id^="formily-item-"], .gh-apply-form__field, [class*="field-entry"], [class*="formField"], fieldset')]
    .filter((i) => !i.querySelector('[id^="formily-item-"]'));
  for (const item of items) {
    const label = (item.querySelector('label, legend, [class*="item-label"]')?.textContent || "").trim().replace(/\\s+/g, " ").slice(0, 60);
    ...
    const ticked = [...item.querySelectorAll('input[type=checkbox]:checked, input[type=radio]:checked')].length;
    const values = [...typed, ...selects, ...shown].join(" | ");
    if (label) out.push(\`\${values ? "  =" : "  ✗"} \${label}\${values ? " => " + values : ""}\${ticked ? " [" + ticked + " ticked]" : ""}\`);
  }
  return out;
})()`);
console.log("FIELDS:");
